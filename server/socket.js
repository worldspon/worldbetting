...
class GatewaySocket {
  constructor(io, uuid) {
    // 게이트웨이 소켓 설정
    // 운영
    this.gateWaySocket = gatewayNet.connect({
      port: 16001,
      host: "202.230.143.223"
    });
    ...
    this.gateWaySocket.on("data", data => {
      ...
      const { TCPSocketIp, TCPSocketPort } = getTCPSocket(
        data,
        this.gateWaySocket
      );
      console.log("TCPSocketIP :", TCPSocketIp);
      console.log("TCPSocketIP :", TCPSocketPort);
