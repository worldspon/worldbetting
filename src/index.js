...
class Index extends React.Component {
    ...
    if (errorCode === 0) {
      ...
      switch (errorCode) {
        ...
        case 250:
          ...
          this.state.webSocket.emit(
            "disconnect",
            this.props.langPack.alert.connectErrorDB
          );
          sessionStorage.clear();
          location.href = "/login";
          break;
  ...
  tryLogout() {
    ...
    const content = encodeUTF16(
      `${this.props.uniqueId}|${this.props.trademark}|`
    );
    const endSignal = encodeUTF8("<End>");
    const sendData = command.concat(content).concat(endSignal);
    this.state.webSocket.emit("tcpsend", sendData);
    ...
    if (errorCode === 0) {
      ...
      switch (errorCode) {
        ...
        case 3:
          ...
          break;
        case 4:
          alert(this.props.langPack.alert.errorAppliedChargeExchange);
          break;
        case 250:
  ...
  responseChargeExchangeListCount(byteArray) {
    ...
    console.log(content);
    this.setState({
      chargeExchangeListEndPage:
        content[0] == 0 ? 0 : Math.ceil(parseInt(content[0]) / 10) - 1
    });
  }
  ...
  responseChargeExchangeList(byteArray) {
    ...
    const errorCode = parseInt(byteArray.slice(0, 1));
    const chargeExchangeListByteStore = [
      ...this.state.chargeExchangeListByteStore
    ];
        ...
        <div className={common.main}>
          ...
          {this.state.chargeModal && (
            <Charge
              ...
              requestChargeExchangeListCount={type =>
                this.requestChargeExchangeListCount(type)
              }
              requestChargeExchangeList={(type, page) =>
                this.requestChargeExchangeList(type, page)
              }
  ...
  <Index
    ...
    langPack={
      sessionStorage.getItem("lang") === null
        ? LanguagePack.ko.lobby
        : LanguagePack[sessionStorage.getItem("lang")].lobby
    }
  />,
