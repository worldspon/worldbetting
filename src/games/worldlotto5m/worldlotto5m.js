...
import styles from "./worldlotto5m.css";
// 로그인 검증
...
class WorldLotto5M extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      staticLimitBettingMoney: 10000000000,
      logoURL: "../../../images/worldlotto5m_game_icon.png",
      apiURL: "worldBall5",
      userPoint: 0,
      userCommission: 0,
      userBonus: 0,
      ...
      // 게임 결과테이블 결과부분 텍스트
      gameResultThText: [
        this.props.langPack.worldBetting.tableTh[0],
        this.props.langPack.worldBetting.tableTh[1]
      ],
      bettingAllowStart: 0,
      bettingAllowEnd: 0,
      // 게임 세팅 정보
  ...
  setGameInfo(byteArray) {
    ...
    if (errorCode === 0) {
      ...
      this.setState({
        bettingAllowStart: bettingAllowTime[2],
        bettingAllowEnd: bettingAllowTime[3],
        bettingCancel: cancelAllow[1]
      });
    } else {
      console.log("실패");
    }
  ...
  setUserGameInfo(byteArray) {
    ...
    if (errorCode === 0) {
      ...
      this.setState({
        minMaxBetting: content.slice(20, 40),
        allocation: content.slice(130, 143),
        totalBettingLimit: content[181]
      });
    } else {
      console.log("실패");
    }
