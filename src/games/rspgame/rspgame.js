...
import styles from "./rspgame.css";
// 로그인 검증
...
class RSPGame extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      // 게임 결과테이블 결과부분 텍스트
      gameResultThText: [
        this.props.langPack.rspGame.tableTh[0],
        this.props.langPack.rspGame.tableTh[1]
      ],
      bettingAllowStart: 0,
      bettingAllowEnd: 0,
      // 게임 세팅 정보
  ...
  setGameInfo(byteArray) {
    ...
    if (errorCode === 0) {
      ...
      this.setState({
        bettingAllowStart: bettingAllowTime[10],
        bettingAllowEnd: bettingAllowTime[11],
        bettingCancel: cancelAllow[5]
      });
    } else {
      console.log("실패");
    }
  ...
  setUserGameInfo(byteArray) {
    ...
    if (errorCode === 0) {
      ...
      this.setState({
        minMaxBetting: content.slice(100, 120),
        allocation: content.slice(170, 180),
        totalBettingLimit: content[185]
      });
    } else {
      console.log("실패");
    }
